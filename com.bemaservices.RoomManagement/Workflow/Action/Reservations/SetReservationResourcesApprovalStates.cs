...
namespace com.bemaservices.RoomManagement.Workflow.Actions.Reservations
{
    ...
    [WorkflowAttribute( "Approval State Attribute", "The attribute that contains the reservation resources' approval state.", false, "", "", 1, null,
        new string[] { "com.bemaservices.RoomManagement.Field.Types.ReservationResourceApprovalStateFieldType" } )]
    [ReservationResourceApprovalStateField( "Approval State", "The approval state to use (if Approval State Attribute is not specified).", false, "", "", 2 )]
    [BooleanField( "Ignore Resources With Approval Groups", "Whether to skip updating the statuses of resources with approval groups", true, "", 3 )]
    public class SetReservationResourcesApprovalStates : ActionComponent
    {
        /// <summary>
        ...
        public override bool Execute( RockContext rockContext, WorkflowAction action, Object entity, out List<string> errorMessages )
        {
            ...
            ReservationResourceApprovalState? approvalState = null;
            ...
            {
                approvalState = action.GetWorkflowAttributeValue( approvalStateAttributeGuid.Value ).ConvertToEnumOrNull<ReservationResourceApprovalState>();
            }

            if ( approvalState == null )
            {
                approvalState = GetAttributeValue( action, "ApprovalState" ).ConvertToEnumOrNull<ReservationResourceApprovalState>();
            }
            ...
            {
                if ( reservationResource.Resource == null )
                {
                ...
                if ( approvalGroup == null || !ignoreResourcesWithApprovalGroups )
                {
                    var oldValue = reservationResource.ApprovalState;

                    reservationResource.ApprovalState = approvalState.Value;

                    if ( oldValue != reservationResource.ApprovalState )
                    {
                        History.EvaluateChange( changes, String.Format( "{0} Approval State", reservationResource.Resource.Name ), oldValue.ToString(), reservationResource.ApprovalState.ToString() );
                    }
                }
            }
